...
// Pull emails from the xlsx so we send to exactly the uploaded set
const wb = XLSX.readFile("/Users/prabhubalasubramaniam/Downloads/Share techno surge sun.xlsx")
const sh = wb.Sheets[wb.SheetNames[0]]
const xlsxRows = XLSX.utils.sheet_to_json(sh, { defval: null })
const TARGET_EMAILS = xlsxRows
  .map(r => String(r["MAIL ID"] || "").trim().toLowerCase())
  .filter(Boolean)
...
console.log(`Target emails (xlsx): ${TARGET_EMAILS.length}`)
console.log(`Matched in DB:        ${regs.length}\n`)

const matchedEmails = new Set(regs.map(r => (r.attendee_email || "").toLowerCase()))
const missing = TARGET_EMAILS.filter(e => !matchedEmails.has(e))
if (missing.length) {
