...
- `message`: always `"Check-in successful!"`, and `alreadyCheckedIn`: always `false`. In the implementation, `scan_id` is only ever attached to a row at insert time (step 6) — it is never backfilled onto a pre-existing "already checked in" row (step 5's `existing` branch never touches `scan_id`). So a `scan_id` hit can only ever be an echo of an original fresh insert; the "already checked in" case this sentence used to describe as a possible replay outcome can't actually occur — there is no code path that produces a row with both a non-null `scan_id` and an "already checked in" origin.
