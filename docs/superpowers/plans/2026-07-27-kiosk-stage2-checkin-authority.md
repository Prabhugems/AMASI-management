...
- **Final review, two rounds:** the whole-branch review found 2 Critical + 4 Important findings, all fixed and re-reviewed clean (see `.superpowers/sdd/2026-07-27-kiosk-stage2-checkin-authority/progress.md` for the full ledger). One of the Critical fixes (filtering `/api/kiosk/delegates`' roster by list eligibility) itself reintroduced an unbounded-query bug in that same route, caught by a second re-review round and fixed. Net: this stage added a list-eligibility gate to `/api/kiosk/checkin`, a matching filter to the delegate cache that feeds it, a temporary one-release compatibility fallback for kiosk tabs running pre-this-stage JS, and a fix for a `scan_id`-uniqueness race that the sync worker's lack of an in-flight guard made reachable.
- **Known, deliberately out-of-scope risk discovered during final review — needs its own task before AMASICON (2026-08-27 to 2026-08-30):** `/api/kiosk/delegates/route.ts`'s base `registrations` roster query (the one that actually builds the kiosk's delegate cache — not the addon-eligibility subquery, which this stage's final review already batched) has no pagination at all. Supabase's default query cap is ~1,000 rows; AMASICON is expected to have ~2,000 delegates. Past the cap, delegates would silently never enter the kiosk's local cache — "Registration not found" at the tablet, no error, no admin visibility, indistinguishable from the exact failure class this whole stage and its final review were about. This predates this branch (Stage 1, PR #119), was not introduced here, and was not fixed here — flagged for a dedicated task, not a Stage 2 follow-up commit.
- **Minor findings from the final review, deferred (not fixed), for whoever revisits this route:** `wonByTwin`'s twin-detection (the `scan_id`-race fix) confirms a row exists but doesn't verify it belongs to the same registration the request expected — a very narrow multi-tab-same-device scenario could return the wrong person's data; the `scan_id`/`station_id` fallback generator (`kiosk-offline-store.ts`'s `newId()`) uses `Math.random()` rather than `crypto.getRandomValues`, worth strengthening since `scan_id` doubles as the sole credential for a PII lookup on a public endpoint; Task 1's distinct "list not found" vs. "registration not found" 404 messages let a caller distinguish which reason applied (mild, pre-existing-adjacent enumeration surface); the temporary Fix 4 compatibility shim (missing-`registration_id` fallback) has no dated removal marker, so nothing will prompt its removal once every kiosk has picked up this stage's client change.
