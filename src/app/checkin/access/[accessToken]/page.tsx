...
export default function StaffCheckinPage() {
  ...
  // Auto-refresh stats every 10 seconds so the stats bar stays current
  // across multiple staff devices scanning simultaneously.
  useEffect(() => {
    ...
    const refreshStats = async () => {
      try {
        ...
        // Silently ignore — stats will refresh on the next interval.
      }
    }

    const intervalId = setInterval(refreshStats, 10_000)
    return () => clearInterval(intervalId)
  }, [checkinList, accessToken])
  ...
  }, [checkinList, accessToken, identity, refreshQueueCount, debugMode, soundEnabled, checkoutMode])
        ...
        <div className="flex items-center justify-between max-w-2xl mx-auto">
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              title={checkoutMode ? "Switch to Check-in mode" : "Switch to Check-out mode"}
            >
              {checkoutMode ? <LogOut className="w-5 h-5" /> : <LogIn className="w-5 h-5" />}
            </button>
            <button
