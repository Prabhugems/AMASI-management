...
const sidebarItems = [
  ...
  { title: "All Registrations", href: "/list", icon: Users },
...
export default function RegistrationsLayout({ children }: { children: React.ReactNode }) {
  ...
  const { data: userPermissions, isLoading: permissionsLoading } = useQuery({
    queryKey: ["user-permissions-registrations", eventId],
    queryFn: async () => {
      ...
      // Event-scoped users need registrations permission, non-event-scoped admins get full access
      const hasAccess = (!isEventScoped && isAdmin) || hasFullAccess || hasRegistrationsPermission
    ...
    staleTime: 0, // Don't cache to ensure fresh permissions
    ...
    return (
      <div className="flex h-[calc(100vh-6rem)] items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-muted-foreground mb-4">You don't have permission to access Registrations.</p>
        ...
        <div className="flex flex-1 overflow-hidden">
          <div className="w-56 border-r bg-muted/30 flex flex-col">
            ...
            <div className="px-4 py-3 border-b">
              <h2 className="font-semibold text-lg">Registrations</h2>
              <p className="text-xs text-muted-foreground">Manage attendees</p>
            </div>
            ...
            {!permissionsLoading && !userPermissions?.hasFullAccess && (
              <div className="p-3 border-t">
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  <ClipboardList className="h-3 w-3" /><span>Registrations Access</span>
                </div>
              </div>
            )}
          </div>
