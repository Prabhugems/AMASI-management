...
export default function BadgesLayout({ children }: { children: React.ReactNode }) {
  ...
  const { data: userPermissions, isLoading: permissionsLoading } = useQuery({
    queryKey: ["user-permissions", eventId, "badges"],
    queryFn: async () => {
      ...
      const hasBadgesPermission = teamMember.permissions?.includes("badges")
      ...
      return {
        ...
        isTeamUser,
        hasFullAccess,
        hasAccess: isAdmin || hasFullAccess || hasBadgesPermission,
        userName: teamMember.name || session.user.email,
      }
    },
    staleTime: 0,
        ...
        <div className="flex flex-1 overflow-hidden">
          <div className="w-56 border-r bg-muted/30 flex flex-col">
            ...
            <div className="px-4 py-3 border-b">
              ...
              <p className="text-xs text-muted-foreground">Design & print badges</p>
            </div>
            ...
            {!permissionsLoading && !userPermissions?.hasFullAccess && (
              <div className="p-3 border-t">
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  <Award className="h-3 w-3" /><span>Badges Access</span>
                </div>
              </div>
            )}
          </div>
