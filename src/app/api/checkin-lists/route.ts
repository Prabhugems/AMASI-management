...
export async function POST(request: NextRequest) {
  try {
    ...
    // Set the staff access-token expiry from the event end (+2 day grace),
    // falling back to 30 days when the event has no end_date. The access_token
    // value itself is auto-generated by a BEFORE INSERT trigger.
    const { data: ev } = await (supabase as any)
      .from("events").select("end_date").eq("id", event_id).single()
    const access_token_expires_at = new Date(
      ev?.end_date ? new Date(ev.end_date).getTime() + 2 * 864e5 : Date.now() + 30 * 864e5
    ).toISOString()
