    ...
    if (data && data.length > 0) {
      ...
      const parseFacultyText = (text: string | null): Array<{name: string, email: string | null, phone: string | null}> => {
        ...
        return text.split(' | ').map(part => {
          // Same fix as program/sync-assignments and program/ai-import: the
          // `\s*` after ([^(]+) overlapped with it (both match whitespace),
          // forcing the engine to try every split of a whitespace run —
          // quadratic. Captures are identical once .trim() below is applied.
