...
// portal URL links delegates to /my where the cert is generated on demand.
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.error("[certificates/whatsapp] Failed to log message:", (logError as Error).message)
    }
    ...
    // Mark cert-ready WhatsApp sent on the registration so the admin UI can show status.
    await db
      .from("registrations")
      .update({
        custom_fields: {
          ...(registration.custom_fields || {}),
          certificate_whatsapp_sent: true,
          certificate_whatsapp_sent_at: new Date().toISOString(),
        },
      })
    ...
    console.error("[certificates/whatsapp] error:", message)
