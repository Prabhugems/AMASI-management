...
export async function GET(request: Request) {
  ...
  const run = await logCronRun("reassign-reviews")
  try {
    ...
    const totalReassigned = results.reduce((sum, r) => sum + Math.max(0, r.reassigned_count), 0)
    await run.ok({ syncedCount: totalReassigned, metadata: { processed: settings.length, results } })
    ...
    console.error("Cron reassign-reviews error:", error)
    await run.err(error)
